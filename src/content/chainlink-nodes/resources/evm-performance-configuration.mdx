...
<Aside type="note" title="Note on Ethereum clients">
  Ethereum clients have bugs. Much work is done on the Chainlink node software to mitigate bugs in various different RPC
  implementations. This guide helps you understand how to mitigate and work around these bugs.
</Aside>
...
<Aside type="caution">Do not modify these values unless _all_ primary nodes are configured with HTTP URLs.</Aside>
...
<Aside type="caution">
  If you increase `ETH_MAX_IN_FLIGHT_TRANSACTIONS` you must make sure that your ETH node is configured properly
